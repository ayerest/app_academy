...
def price_is_right(bids, actual_retail_price)
    ...
    #         if sorted[i] == actual_retail_price
    #             price = sorted[i]
    #         elsif sorted[i] < actual_retail_price && sorted[i + 1] > actual_retail_price
    #             price = sorted[i]
    #         end
    #         i += 1
    #     end
    # return price
    best_bids = bids.reject { |bid| bid > actual_retail_price }
    best_bids.max
end
...
def ordered_vowel_word?(word)
    ...
    #     end
    # end
    hasVowels = word.chars.select do |letter|
        vowels.include?(letter)
    end
    hasVowels == hasVowels.sort
end
...
def products_except_me(numbers)
    numbers.map.with_index do |num, index|
        newArr = numbers[0...index] + numbers[(index + 1)..-1]
